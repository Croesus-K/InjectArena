...
test('levels/ 全部关卡通过 schema 校验且为 L1-L4', () => {
  const levels = loadLevels(path.join(ROOT, 'levels'));
  assert.deepEqual(levels.map((l) => l.id), ['L1', 'L2', 'L3', 'L4']);
...
test('corpus/ 语料库通过 schema 校验', () => {
  ...
  assert.deepEqual(corpora.map((c) => c.id), ['data-exfiltration', 'direct-injection', 'indirect-injection']);
});

test('语料库规模与质量约束：直接注入 ≥50、数据窃取 ≥15、间接注入 ≥12、id 唯一、中英混合', () => {
  const corpora = loadCorpus();
  ...
  assert.ok(payloads.length >= 77, '语料总量至少 77 条（当前 ' + payloads.length + '）');
