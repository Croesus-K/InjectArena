...
test('levels/ 全部关卡通过 schema 校验且为 L1-L6', () => {
  ...
  for (const lv of levels) {
    ...
    assert.ok(lv.debrief.defenseSample && lv.debrief.defenseSample.length >= 40, '复盘须附可直接粘贴的防护参考');
    assert.ok(Array.isArray(lv.defenseTemplates) && lv.defenseTemplates.length >= 3, '每关至少 3 个防护模板（一键套用库）');
