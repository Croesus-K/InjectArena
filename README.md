...
- **守方（段位榜数据源）**：每关有「布防插槽」，写入防护提示词后可用内置语料库跑分（v1 引擎已就绪，评分入口后续会话接入）。
...
├── corpus/              # 攻击 payload 语料库（schema + 直接注入 30 条）
...
```bash
npm test        # node:test，57 项：引擎纯逻辑 + provider(mock fetch) + server(fastify inject)
```
...
- [ ] L3 对抗防护（关键词过滤变形绕过）、L4 间接注入（RAG 投毒）、L5 工具滥用
- [ ] 布防插槽评分入口（引擎已就绪：`src/defenseEvaluator.js`）
- [ ] 名将榜 / 段位榜（引擎已产出纯数据报告）
- [ ] 语料库扩充至 50+ 条、新增间接注入语料
